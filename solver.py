# -*- coding: utf-8 -*-
"""
Created on Tue Feb 25 07:14:41 2025
...
# Extract driver IDs and depots
drivers = df_availability["Driver"].tolist()
depots = df_availability["Depot"].tolist()
...
# Maximum weekly hours per driver (35 hours)
max_weekly_hours = 35
    ...
    for day in days:
        for shift in shifts:
            if x[d, day, shift].varValue == 1:
                schedule.append([d, depots[drivers.index(d)], day, shift, shift_hours])  # Assigned shift (7 hrs)
                total_hours += shift_hours

# Convert to DataFrame
df_schedule = pd.DataFrame(schedule, columns=["Driver", "Depot", "Day", "Shift", "Hours"])
